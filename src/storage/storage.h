...
class MoreporkFileInfo : public QObject {
  Q_OBJECT
  ...
  Q_PROPERTY(float extrusionMassGramsA
             READ extrusionMassGramsA
             NOTIFY fileInfoChanged)
  Q_PROPERTY(float extrusionMassGramsB
             READ extrusionMassGramsB
             NOTIFY fileInfoChanged)
  Q_PROPERTY(int extruderTempCelciusA
             READ extruderTempCelciusA
             NOTIFY fileInfoChanged)
  Q_PROPERTY(int extruderTempCelciusB
             READ extruderTempCelciusB
             NOTIFY fileInfoChanged)
  Q_PROPERTY(int chamberTempCelcius
             READ chamberTempCelcius
             NOTIFY fileInfoChanged)
  ...
  public:
    ...
    MoreporkFileInfo(QObject *parent = 0) : QObject(parent) { }
    MoreporkFileInfo(
    ...
    MoreporkFileInfo& operator=(const MoreporkFileInfo& rvalue){
      MoreporkFileInfo* temp =
        new MoreporkFileInfo(rvalue.file_path_,
                             rvalue.file_name_,
    ...
    #define C_MPFI_PTR_T const MoreporkFileInfo*
    static bool fileNameLessThan(const QObject *a, const QObject *b){
      return static_cast<C_MPFI_PTR_T>(a)->fileName() <
             static_cast<C_MPFI_PTR_T>(b)->fileName();
    }

    static bool accessDateGreaterThan(const QObject *a, const QObject *b){
      return static_cast<C_MPFI_PTR_T>(a)->fileLastRead().toSecsSinceEpoch() >
             static_cast<C_MPFI_PTR_T>(b)->fileLastRead().toSecsSinceEpoch();
    }

    static bool timeEstimateSecLessThan(const QObject *a, const QObject *b){
      return static_cast<C_MPFI_PTR_T>(a)->timeEstimateSec() <
             static_cast<C_MPFI_PTR_T>(b)->timeEstimateSec();
    }
    #undef C_MPFI_PTR_T
...
class ThumbnailPixmapProvider : public QQuickImageProvider {
  ...
  public:
    ...
    QPixmap requestPixmap(const QString &absolute_file_path,
                          QSize *size,
...
class MoreporkStorage : public QObject {
  ...
  public:
    ...
    Q_INVOKABLE void updatePrintFileList(const QString directory);
    Q_INVOKABLE void deletePrintFile(QString file_name);
    Q_PROPERTY(QList<QObject*> fileList
               READ fileList
               WRITE fileListSet
               RESET fileListReset
               NOTIFY fileListChanged)
    QList<QObject*> fileList() const;
    void fileListSet(const QList<QObject*> &file_list);
    void fileListReset();
    Q_INVOKABLE void updateCurrentThing(bool is_test_print = false);
    Q_PROPERTY(MoreporkFileInfo* currentThing
               READ currentThing
               WRITE currentThingSet
               RESET currentThingReset)
    MoreporkFileInfo* currentThing() const;
    void currentThingSet(MoreporkFileInfo* current_thing);
